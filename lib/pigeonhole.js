...
require('./pigeonhole/array.js');

const Trigger = require('./pigeonhole/trigger.js');
const fs = require('fs');
...
class Pigeonhole {
  ...
  evaluateWithCategoryIds(record, categoryIds) {
  ...
  // NOTE: It's important that each id ends with a separator
  //       otherwise '123-45' will match '123-456'
  dasherize(array) {
    return array.join('-') + '-'
  }
  ...
  convertPathsToIds(paths) {
    ...
    let finalIds = deepest.map(string => {
      let ids = string.split('-')
      return Number(ids[ids.length - 2])
    })
